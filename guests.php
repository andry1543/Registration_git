    ...
    <body>
        ...
        <div class="form">
           <FORM method= "GET" action="organizations.php">
               <INPUT type="submit" name = button class ="button" value= "Поиск людей по организациям">
           </FORM>
        </div>
        <table id="main" class="main">
            <tr>
                <td >
                    <table  id="guests" class="sort">
                        ...
                        <tbody>
                            ...
                            <?php
                                include "header.php";
                                $query = $db->query("SELECT * FROM ".$dbtable." WHERE Visited = 1 and (Organization not like 'RDIF' or Organization is NULL)");
                                $i = 0;    
                                while ($row = $query->fetch_assoc()) {
                                    if ($row['Surname_en'] != '' or $row['Name_en'] != '')
                                        echo '<tr><td><h3>'.$row['RegDate'].'</h3></td><td><h3><a href = "../../index.php?guestid='.$row['ID'].'">'.$row['Surname_en'].' '.$row['Name_en'].'</a></h3>
                                                 </td><td><h3><a href = "../../organizations.php?org='.$row['Organization'].'">'.$row['Organization'].'</h3></td></tr>';
                                    else
                ...
                <td>
                    <table id="rdif" class="sort">
                        <caption><h1>Сотрудники РФПИ</h1></caption>
                        <thead>
                            <tr>
                                <td>Дата регистрации</td>
                                <td>Сотрудник</td>
                        ...
                        <tbody>
                            <?php
                                $query = $db->query("SELECT * FROM ".$dbtable." WHERE Visited = 1 and Organization like 'RDIF'");
                                $i = 0;    
                                while ($row = $query->fetch_assoc()) {
                                    if ($row['Surname_en'] != '' or $row['Name_en'] != '')
                                        echo '<tr><td><h3>'.$row['RegDate'].'</h3></td><td><h3><a href = "../../index.php?guestid='.$row['ID'].'">
                                        '.$row['Surname_en'].' '.$row['Name_en'].'</a></h3></tr>';
                                    else
                                        echo '<tr><td><h3>'.$row['RegDate'].'</h3></td><td><h3><a href = "../../index.php?guestid='.$row['ID'].'">
                                        '.$row['Surname_ru'].' '.$row['Name_ru'].'</a></h3></tr>';
                                    $i++;
                                }
                            ?>
                        </tbody>
